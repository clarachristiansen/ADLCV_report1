...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        keys = rearrange(keys, 'B seq_len (num_heads head_dim) -> B seq_len num_heads head_dim', 
                         num_heads=self.num_heads,head_dim=self.head_dim) 
        # Secondly merge the batch_size with the num_heads
        keys = rearrange(keys, 'B seq_len num_heads head_dim -> (B num_heads) seq_len head_dim')
        
        # HINT repeat the same process for queries and values
        queries = rearrange(queries, 'B seq_len (num_heads head_dim) -> B seq_len num_heads head_dim', 
                         num_heads=self.num_heads,head_dim=self.head_dim)
        queries = rearrange(queries, 'B seq_len num_heads head_dim -> (B num_heads) seq_len head_dim')

        values = rearrange(values, 'B seq_len (num_heads head_dim) -> B seq_len num_heads head_dim', 
                         num_heads=self.num_heads,head_dim=self.head_dim)
        values = rearrange(values, 'B seq_len num_heads head_dim -> (B num_heads) seq_len head_dim')
        # Compute attetion logits
        attention_logits = torch.matmul(queries, rearrange(keys, 'batch_size seq_len head_dim -> batch_size head_dim seq_len')) # multiply queries and keys
